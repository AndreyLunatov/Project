...
Сайт: **http://localhost:5173**. Бэкенд должен быть запущен отдельно (`cd ../backend && dotnet run`), порт по умолчанию
**5248**.
...
- В **разработке** запросы идут на **относительный** базовый путь `/api` (см. `src/api.js`) — Vite сам проксирует на
  ASP.NET.
- Пример бэкенда: `GET /api/Health/status`.
- В **продакшене** задайте базу через переменную окружения при сборке: `VITE_API_BASE` (например
  `https://api.example.com/api`). Если не задана, используется `/api` (один origin с API или свой reverse proxy).
...
Ожидаются origin’ы **http://localhost:5173** и **http://localhost:3000** (настройка в `backend/Program.cs`). Менять порт
фронта — синхронизировать с бэкендом.
