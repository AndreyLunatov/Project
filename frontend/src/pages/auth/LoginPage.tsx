...
import type {ILogin} from "../../utils"; // ← добавь
...
export default function LoginPage({submitForm}: ILogin) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await submitForm(login, password); // ждём завершения, но не переключаем страницу здесь – переключит App
    navigate('/');
  };
        ...
        <div className={'w-full md:w-1/2 p-12 flex flex-col justify-center'}>
          ...
          <form className={'flex flex-col gap-6 mb-6'} onSubmit={handleSubmit}>
            ...
            <div className={'flex flex-col gap-6'}>
              <button className={'px-4 py-2 bg-accent w-full h-12 rounded-xl cursor-pointer'} type={'submit'}>
                Войти
              </button>
              <button
                className={'cursor-pointer text-sm text-gray-600 hover:text-gray-900'}
                type={'button'}
                onClick={() => navigate('/auth/register')} // ← теперь работает
              >
